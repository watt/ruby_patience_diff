...
b = %w{ c d e b x x h j e f }

puts "a:"
puts a.map.with_index { |char, index| "#{index.to_s.rjust(4)}: #{char}\n" }
puts "b:"
puts b.map.with_index { |char, index| "#{index.to_s.rjust(4)}: #{char}\n" }

p = Patience.new
answer = []
p.recurse_matches(a, b, 0, 0, a.length, b.length, answer)
print_tuples(answer)

seq = p.collapse_sequences(answer)
print_tuples(seq)

puts "diff:"
p.unified_diff(a, b).each do |line|
  puts line
end
